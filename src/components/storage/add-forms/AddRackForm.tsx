...
interface AddRackFormProps {
  parentAisle: StorageNode;
  parentChain?: StorageNode[];
  onSave: (rackData: Partial<StorageNode>) => void;
  onCancel: () => void;
}

export function AddRackForm({ parentAisle, parentChain = [], onSave, onCancel }: AddRackFormProps) {
  ...
  const handleNameChange = (value: string) => {
    setFormData(prev => ({
      ...prev,
      name: value,
      code: value ? `RK-${value.substring(0, 2).toUpperCase()}-${Date.now().toString().slice(-3)}` : ''
    }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (bulkCreate) {
      ...
      for (let i = 1; i <= count; i++) {
        const rackData: Partial<StorageNode> = {
          type: 'rack',
          name: `${bulkPrefix} ${i}`,
          code: `RK-${bulkPrefix.substring(0, 2).toUpperCase()}${i}`,
          description: formData.description,
          status: formData.status,
          parentId: parentAisle.id,
        };
        onSave(rackData);
      }
    } else {
    ...
    <div className="flex flex-col h-full">
          ...
          <div className="flex items-center gap-1.5 mb-2 text-xs text-gray-600 overflow-x-auto">
            {parentChain.length > 0 ? (
              <>
                ...
                <span className="text-[#5C1F3D] font-medium">Add Rack</span>
              </>
            ) : (
              <>
                <span className="text-gray-700">{parentAisle.name}</span>
                <ChevronRight className="w-3 h-3 text-gray-400" />
                <span className="text-[#5C1F3D] font-medium">Add Rack</span>
              </>
            )}
          </div>
          <h3 className="text-base text-gray-900">Add New Rack</h3>
          <p className="text-xs text-gray-500 mt-1">
            Create new rack under {parentAisle.name}
          </p>
        </div>
        <button
          onClick={onCancel}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-gray-700">Create multiple racks at once</span>
            </label>
          </div>
          ...
          {bulkCreate ? (
            ...
            <>
              <div>
                <label className="block text-sm text-gray-700 mb-2">
                  Rack Name Prefix <span className="text-red-500">*</span>
                </label>
                <StyledTextField
                  value={bulkPrefix}
                  onChange={(e) => setBulkPrefix(e.target.value)}
                  placeholder="e.g., Rack, R"
                  className="w-full"
                  required
                />
                <p className="text-xs text-gray-500 mt-1">
                  Racks will be named: {bulkPrefix} 1, {bulkPrefix} 2, etc.
                </p>
              </div>
            ...
            <>
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  Auto-generated based on rack name
                </p>
              </div>
            </>
          )}
          ...
          <div>
            ...
            <StyledSelect
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, rackType: e.target.value }))}
              className="w-full"
            ...
            >
              ...
              <MenuItem value="heavy-duty" style={{ fontSize: 'var(--text-sm)', textAlign: 'left' }}>Heavy Duty</MenuItem>
              <MenuItem value="cantilever" style={{ fontSize: 'var(--text-sm)', textAlign: 'left' }}>Cantilever</MenuItem>
              <MenuItem value="pallet" style={{ fontSize: 'var(--text-sm)', textAlign: 'left' }}>Pallet</MenuItem>
            </StyledSelect>
          </div>
      ...
      <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-end gap-3 bg-gray-50 flex-shrink-0">
        ...
        <button
          ...
          style={{ height: '33px' }}
        >
          {bulkCreate ? `Create ${bulkCount} Racks` : 'Create Rack'}
        </button>
      </div>
    </div>
  );
